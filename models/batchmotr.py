...
class MOTR(nn.Module):
    ...
    def forward(self, data: dict):
        ...
        features, pos = self.backbone(frames)  # 通过backbone获取多次度feature+pad mask+position
        src, mask = features[-1].decompose()
        assert mask is not None
        ...
        for frame_index, (gt, proposals) in enumerate(zip(data['gt_instances'], data['proposals'])):
            ...
            if self.query_denoise > 0: # dn-detr方式训练使用，制作一批噪声GT训练decode
            ...
            src_cur = [src[frame_index:frame_index+1] for src in srcs]
            mask_cur = [src[frame_index:frame_index+1] for src in masks]
            pos_cur = [src[frame_index:frame_index+1] for src in pos]
            
            # transformer 包括encode + decode
            hs, init_reference, inter_references, enc_outputs_class, enc_outputs_coord_unact = \
                self.transformer(src_cur, mask_cur, pos_cur, query_embed, ref_pts=ref_pts,
                                mem_bank=track_instances.mem_bank, mem_bank_pad_mask=track_instances.mem_padding_mask, attn_mask=attn_mask)
            outputs_classes = []
            outputs_coords = []
            for lvl in range(hs.shape[0]):  # 检测头包括box（xc,yc,w,h并归一化1）和class分支
            ...
            frame_res = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1]}  # 最后一次迭代的输出
            if self.aux_loss:
                frame_res['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord) # 中间过程迭代的输出
            frame_res['hs'] = hs[-1]
