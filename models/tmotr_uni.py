...
class ClipMatcher(SetCriterion):
    ...
    def get_loss(self, loss, outputs, gt_instances, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'labels_mask': self.loss_labels_withmask
        }
    ...
    def loss_labels_withmask(self, outputs, gt_instances: List[Instances], indices, num_boxes, log=False):
        """Classification loss (NLL)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
        src_logits = outputs['pred_logits']
        ...
        if self.focal_loss:
            ...
            loss_ce = sigmoid_focal_loss(src_logits.flatten(1),
                                             ...
                                             alpha=0.25,
                                             gamma=2,
                                             num_boxes=num_boxes, mean_in_dim1=False, issum=False)
            mask_valid = torch.full(src_logits.shape[:2], 0, dtype=torch.float, device=src_logits.device)
            mask_valid[idx] = 1
            loss_ce = loss_ce.reshape(src_logits.shape) * mask_valid[..., None]
            loss_ce = loss_ce.sum()
        else:
            loss_ce = F.cross_entropy(src_logits.transpose(1, 2), target_classes, self.empty_weight)
        losses = {'loss_ce': loss_ce}
    ...
    def match_for_single_frame(self, outputs: dict):
        ...
        for loss in self.losses:
            if loss == 'labels' and gt_instances_i._data_name == 'TAO': 
                loss='labels_mask'
